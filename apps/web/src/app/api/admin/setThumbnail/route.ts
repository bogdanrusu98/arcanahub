...
// Dacă folosești Buffer/Node APIs pe Vercel, asigură-te de runtime:
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!videoId || !maybeFile || !(maybeFile instanceof File)) {
    ...
    await gcsFile.save(buffer, {
      ...
      resumable: false,
      public: process.env.FIREBASE_PUBLIC_FILES === "true", // controlează public din env
      metadata: {
        cacheControl: "public, max-age=31536000, s-maxage=31536000, immutable",
      },
    });

    // Prefer public URL if bucket allows it; otherwise signed URL
    const url =
      process.env.FIREBASE_PUBLIC_FILES === "true"
